...
public class Solution {
    ...
    public static Stream<Car> onlyElectricCars(ArrayList<Car> cars) {
        Stream<Car> carStream1 = cars.stream().filter(electro -> (electro.isElectric())); //
        // применяем метод filter каторый фильтрует
        // обьекты по параметрам "electro"  и выводит только электро машины
